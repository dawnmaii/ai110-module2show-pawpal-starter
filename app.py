...
if st.button("Generate schedule"):
    ...
    if not selected_pets:
        ...
        if not plan:
            ...
            st.session_state.filter_pet = None
            st.success(f"Scheduled {len(plan)} task(s) in {scheduler._minutes_used} / {available_minutes} min.")
...
if st.session_state.last_plan:
    ...
    with col1:
        ...
        filter_priority = st.multiselect("Filter by priority", priority_options, default=saved_priority)
    ...
    with col2:
        ...
        filter_pet = st.multiselect("Filter by pet", pet_options, default=saved_pet)
    ...
    edited_schedule = st.data_editor(
        ...
        disabled=["time", "task", "pet", "duration (min)", "priority"],
        hide_index=True,
        width="stretch",
    )
            ...
            if next_task:
                ...
                if pet:
                    try:
                        ...
                        st.info(f"'{task.name}' marked complete. Next occurrence scheduled for {next_task.due_date}.")
                    except ValueError:
